...
async function buildPackage() {
  if (program.build) {
    ...
    await execute(cmd).catch(error => {
      console.log(`Failed to build @nwx/gtag ... ${error}`);
      return false;
    });
  }
  return true;
...
program
  ...
  .option('-b, --build', 'Build @nwx/gtag')
  .option('-p, --publish', 'Publish @nwx/gtag@latest')
  .option('-d, --dev', 'Publish @nwx/gtag@dev')
  .parse(process.argv);
